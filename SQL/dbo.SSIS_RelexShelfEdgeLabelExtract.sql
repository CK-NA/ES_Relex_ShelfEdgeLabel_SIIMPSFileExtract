...
BEGIN
	...
	DECLARE @sourceFiles TABLE (ID INT IDENTITY, sourceFileName VARCHAR(250));
	...
	DECLARE @lastInsertedDate VARCHAR(50),
			@sourceFileName varchar(250);
	SELECT @lastInsertedDate = MAX(sys_inserted_dt) FROM dbo.relex_outbound_shelf_edge_labels;
	...
	WHERE sys_inserted_dt = @lastInsertedDate;

	SELECT @count = COUNT(ID) FROM @sourceFiles;

	WHILE @iterator < @count
	BEGIN

		-- Set our iterator record
		SET @iterator = @iterator + 1;

		SELECT @sourceFileName = sourceFileName FROM @sourceFiles WHERE ID = @iterator
		...
		WHERE l.source_file_name = @sourceFileName
